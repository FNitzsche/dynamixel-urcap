package de.awesome_technologies.urcap.dynamixel.dynamixelSDK;
/*******************************************************************************
...
public class ReadWrite {
  ...
  short[] dxl_goal_position = new short[]{DXL_MINIMUM_POSITION_VALUE, DXL_MAXIMUM_POSITION_VALUE};
  byte dxl_error = 0;                                        // Dynamixel error
  short dxl_present_position = 0;                            // Present position

  public void setVariables(DataModel dataModel){
    ADDR_MX_TORQUE_ENABLE = (short) dataModel.get("ADDR_MX_TORQUE_ENABLE", 24);
    ADDR_MX_GOAL_POSITION = (short) dataModel.get("ADDR_MX_GOAL_POSITION", 30);
    ADDR_MX_PRESENT_POSITION = (short) dataModel.get("ADDR_MX_PRESENT_POSITION", 36);
    DXL_ID = (byte) dataModel.get("DXL_ID", 1);
    BAUDRATE = dataModel.get("BAUDRATE", 57600);
    DEVICENAME = dataModel.get("DEVICENAME", "/dev/ttyUSB0");
  }

  public void openConnectionAndEnableMovement(){
    // Instead of getch
  ...
  public void moveToPosition(short targetPosition){
    if (!opened){
      return;
    }

      // Write goal position
      dynamixel.write2ByteTxRx(port_num, PROTOCOL_VERSION, DXL_ID, ADDR_MX_GOAL_POSITION, targetPosition);
      if ((dxl_comm_result = dynamixel.getLastTxRxResult(port_num, PROTOCOL_VERSION)) != COMM_SUCCESS)
      {
        System.out.println(dynamixel.getTxRxResult(PROTOCOL_VERSION, dxl_comm_result));
      }
      ...
      do
      {
        ...
        System.out.printf("[ID: %d] GoalPos:%d  PresPos:%d\n", DXL_ID, targetPosition, dxl_present_position);

      } while ((Math.abs(targetPosition - dxl_present_position) > DXL_MOVING_STATUS_THRESHOLD));
  }

  public void disableMovementAndCloseConnection(){
    if(!opened){
      return;
    }
